...
def price_area(df1):
    df1['coeff'] = np.where(df1['land_surface'] + df1['garden_area'] < 250, 2,
                            np.where(df1['land_surface'] + df1['garden_area'] < 1000, 4,
                                     np.where(df1['land_surface'] + df1['garden_area'] < 5000, 8,
                                              np.where(df1['land_surface'] + df1['garden_area'] < 10000, 12, 16))))
    df1['divisor'] = df1['area'] + df1['terrace_area'] + df1['garden_area'] / df1['coeff'] + df1['land_surface'] / df1[
        'coeff']
...
final_df['status'] = np.where(final_df['building_state_agg'] == 'AS_NEW',
                              (300 * (final_df['area'] + final_df['terrace_area'])),
                              np.where(final_df['building_state_agg'] == 'JUST_RENOVATED',
                                       (150 * (final_df['area'] + final_df['terrace_area'])),
                                       np.where(final_df['building_state_agg'] == 'TO_RENOVATE',
                                                (-150 * (final_df['area'] + final_df['terrace_area'])),
                                                np.where(final_df['building_state_agg'] == 'TO_RESTORE',
                                                         (-300 * (final_df['area'] + final_df['terrace_area'])), 0))))
f = ['open_fire', 'swimming_pool_has', 'furnished', 'equipped_kitchen_has']
...
x_train = final_df.drop(
    ['status', 'price', 'price_area', 'house_is', 'divisor', 'coeff', 'postcode', 'equipped_kitchen_has', 'garden',
     'terrace'], axis=1)
...
random_forest_reg = ensemble.RandomForestRegressor(max_depth=20, random_state=0)
...
extra_trees_reg = ensemble.ExtraTreesRegressor(n_estimators=400, random_state=5)
...
voting_reg = ensemble.VotingRegressor(
    [('lr', LinearRegression()), ('rf', ensemble.RandomForestRegressor(n_estimators=200, random_state=0))])
...
gradient_boosting_reg = ensemble.GradientBoostingRegressor(
    n_estimators=400, max_depth=5, min_samples_split=7, learning_rate=0.1, loss='ls')
gradient_boosting_reg.fit(X_train, y_train)
#print("GradientBoostingRegressor train score: ", gradient_boosting_reg.score(X_train, y_train))
#print("GradientBoostingRegressor test score: ", gradient_boosting_reg.score(X_test, y_test))

# package the model for exporting using pickle
import pickle

model_path = "./model.pkl"
pickle.dump(gradient_boosting_reg, open(model_path, 'wb'))
